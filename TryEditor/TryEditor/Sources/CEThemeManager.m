...
/// TryEditor 1.6 以前からの引き継ぎ作業用カテゴリ。引き継ぎが十分済んだら削除して良い。
...

{
    self = [super init];
    if (self) {
        // TryEditor 1.5までのカラーリング設定の引き継ぎ
        [self migrateTheme];
        
        // バンドルされているテーマの名前を読み込んでおく
        NSArray *URLs = [[NSBundle mainBundle] URLsForResourcesWithExtension:@"trytheme" subdirectory:@"Themes"];
...
//------------------------------------------------------
{
    return [[[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory
                                          ...
                                          appropriateForURL:nil
                                                     create:NO
                                                      error:nil]
            URLByAppendingPathComponent:@"TryEditor/Themes"];
}
...
//------------------------------------------------------
{
    return [[[self userThemeDirectoryURL] URLByAppendingPathComponent:themeName] URLByAppendingPathExtension:@"trytheme"];
}
...
//------------------------------------------------------
{
    return [[NSBundle mainBundle] URLForResource:themeName withExtension:@"trytheme" subdirectory:@"Themes"];
}
...
//------------------------------------------------------
/// TryEditor 1.5以前から CotEdito 1.6 への移行
- (void)migrateTheme
//------------------------------------------------------
{
    // テーマディレクトリがある場合は移行済み
...
//------------------------------------------------------
/// TryEditor 1.5までで使用されていたデフォルトテーマに新たなキーワードを加えたもの
...
//------------------------------------------------------
/// TryEditor 1.5までで使用されていたカラーリング設定のUserDefaultsキーとテーマファイルで使用しているキーの対応テーブル
