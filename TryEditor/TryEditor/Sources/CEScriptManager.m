...
//------------------------------------------------------
{
    ...
    [menuItem setTag:CEScriptMenuDirectoryTag];  // Alternate表示のための修飾キーをTryEditor menu key bindingsの対応外にする
...

{
    ...
    if (![destURL checkResourceIsReachableAndReturnError:nil]) {
        ...
        if (success) {
            ...
            DLog(@"Error. Sample script folder could not be copied.");
        }
...
//------------------------------------------------------
{
    return [[[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory
                                          ...
                                          appropriateForURL:nil
                                                     create:YES
                                                      error:nil]
            URLByAppendingPathComponent:@"TryEditor/ScriptMenu"];
}
...

{
    ...
    while (![scanner isAtEnd]) {
        [scanner scanUpToString:@"%%%{TryEditorXInput=" intoString:nil];
        if ([scanner scanString:@"%%%{TryEditorXInput=" intoString:nil]) {
...

{
    ...
    while (![scanner isAtEnd]) {
        [scanner scanUpToString:@"%%%{TryEditorXOutput=" intoString:nil];
        if ([scanner scanString:@"%%%{TryEditorXOutput=" intoString:nil]) {
            if ([scanner scanUpToString:@"}%%%" intoString:&scannedString]) {
                break;
            }
        }
...
//------------------------------------------------------
{
    ...
    return [CommonUtils keyEquivalentAndModifierMask:modifierMask fromString:keySpec includingCommandKey:YES];
}
