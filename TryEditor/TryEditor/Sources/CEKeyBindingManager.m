...
//------------------------------------------------------
{
    ...
    if (success) {
        ...
        DLog(@"Error on saving the menu keybindings setting file.");
    }
    [self resetAllMenuKeyBindingWithDictionary];
...
//------------------------------------------------------
{
    ...
    if ([self prepareUserSettingDicrectory]) {
        ...
        if (![dictToSave writeToURL:fileURL atomically:YES]) {
            DLog(@"Error! Could not save the Text keyBindings setting file...");
            return NO;
        }
    }
...
//------------------------------------------------------
{
    return [[[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory
                                          ...
                                          appropriateForURL:nil
                                                     create:NO
                                                      error:nil]
            URLByAppendingPathComponent:@"TryEditor/KeyBindings"];
    ...
    if ([self prepareUserSettingDicrectory]) {
            ...
            ![fileURL checkResourceIsReachableAndReturnError:nil])
        {
            if (![[NSFileManager defaultManager] copyItemAtURL:sourceURL toURL:fileURL error:nil]) {
                DLog(@"Error! Could not copy \"%@\" to \"%@\"...", sourceURL, fileURL);
                return;
            }
        }
...
//------------------------------------------------------
{
    ...
    for (NSMenuItem *item in [menu itemArray]) {
        ...
        if ([item hasSubmenu]) {
            ...
            NSString *keyEquivalent = [CommonUtils keyEquivalentAndModifierMask:&modifierMask
