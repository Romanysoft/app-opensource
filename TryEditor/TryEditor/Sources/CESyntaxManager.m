...
@property (nonatomic, retain) NSString* BuildInScDir;
...
- (void) processStyleNames
{
    ...
    self.BuildInScDir = [NSString stringWithFormat:@"%@/plugins/plugin.sc", homeDir];
    NSArray* buildIn_URLs = [fm contentsOfDirectoryAtPath:self.BuildInScDir error:nil];
    
    BOOL havePlist = NO;
    for (NSString* pathUrl in buildIn_URLs) {
        if ([pathUrl hasSuffix:@".plist"]) {
            havePlist = YES;
            break;
        }
    }
    ...
    // 没有找到解压的数据，需要处理
    if (!havePlist) {
        NSURL* scUrl = [[NSBundle mainBundle] URLForResource:@"plugin.sc" withExtension:@"zip"];
        
        NSString* zipFilePath = [scUrl path];
        ZipFile* zFile = nil;
        @try {
            zFile = [[ZipFile alloc] initWithFileName:zipFilePath mode:ZipFileModeUnzip];
            if (zFile) {
                BOOL bContinue = YES;
                while (bContinue) {
                    FileInZipInfo* info = [zFile getCurrentFileInZipInfo];
                    if ([info.name hasSuffix:@".plist"]) {
                        ZipReadStream* readStream = [zFile readCurrentFileInZip];
                        NSMutableData* data = [[NSMutableData alloc] initWithLength:info.length];
                        [readStream readDataWithBuffer:data];
                        [readStream finishedReading];
                        
                        NSString* exportFile = [NSString stringWithFormat:@"%@/%@",self.BuildInScDir,info.name];
                        if (![fm fileExistsAtPath:[exportFile stringByDeletingLastPathComponent]]) {
                            [fm createDirectoryAtPath:[exportFile stringByDeletingLastPathComponent] withIntermediateDirectories:YES attributes:nil error:nil];
                        }
    ...
    buildIn_URLs = [fm contentsOfDirectoryAtPath:self.BuildInScDir error:nil];
    NSMutableArray *styleNames = [NSMutableArray array];
    for (NSString* url_path in buildIn_URLs) {
        if([url_path hasSuffix:@".plist"]){
            [styleNames addObject:[url_path stringByDeletingPathExtension]];
        }
    }
...
//------------------------------------------------------
{
    ...
    if ([URL checkResourceIsReachableAndReturnError:nil]) {
        ...
        if (success) {
            ...
            DLog(@"Error. Could not remove \"%@\".", URL);
        }
    } else {
        DLog(@"Error. Could not be found \"%@\" for remove.", URL);
    }
    return success;
}
...
//------------------------------------------------------
{
    return [[[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory
                                          ...
                                          appropriateForURL:nil
                                                     create:NO
                                                      error:nil]
            URLByAppendingPathComponent:@"TryEditor/plugin.sc"];
}

- (NSURL *)plguinStyleDirctoryURL
{
    NSURL* url =  [NSURL fileURLWithPath:self.BuildInScDir];
    return url;
}
...
//------------------------------------------------------
{
    ...
    if (!success) {
        DLog(@"failed to create a directory at \"%@\".", URL);
    }
...
//------------------------------------------------------
{
    NSURL *url = [[[self plguinStyleDirctoryURL] URLByAppendingPathComponent:styleName] URLByAppendingPathExtension:@"plist"];
    return url;
...
//------------------------------------------------------
{
    ...
    if ([dirURL checkResourceIsReachableAndReturnError:nil]) {
        NSDirectoryEnumerator *enumerator = [[NSFileManager defaultManager]
                                             ...
                                             {
                                                 DLog(@"Error on seeking SyntaxStyle Files Directory.");
                                                 return YES;
                                             }];
