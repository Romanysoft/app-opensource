    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(documentDidFinishOpen:)
                                                     name:BaseDocumentDidFinishOpenNotification object:nil];
...

{
    ...
    NSStringEncoding encoding = [[BaseDocumentController sharedDocumentController] accessorySelectedEncoding];
...

{
    ...
    // # テキスト編集時の改行コードの置換場所
    //  * ファイルオープン = BaseDocument > setStringToEditor
...

{
    ...
    for (NSUInteger i = 0; i < currentLength; i++) {
        ...
        if ([CommonUtils isInvalidYenEncoding:encoding] && [currentChar isEqualToString:yenMarkChar]) {
    ...
    if (!string && (encoding == k_autoDetectEncodingMenuTag)) {
        ...
        for (NSNumber *encodingNumber in encodings) {
            ...
            if ((encoding == NSISO2022JPStringEncoding) && shouldSkipISO2022JP) {
                ...
                DLog(@"encoding == NSProprietaryStringEncoding");
                break;
            }
            string = [[NSString alloc] initWithData:data encoding:encoding];
            if (string) {
...

{
    ...
    @catch (NSException *exception) {
        ...
        data = [NSData dataWithContentsOfFile:convertedPath];
        // presentedItemDidChangeにて内容の同一性を比較するためにファイルのMD5を保存する
        [self setFileMD5:[data MD5]];
    }
    ...
    if (data == nil) {
        // オープンダイアログでのエラーアラートは BaseDocumentController > openDocument: で表示する
...

{
    ...
    @catch (NSException *exception) {
        success = [data writeToFile:convertedPath atomically:YES];
    }
...
{
    if (([string length] > 0) && [CommonUtils isInvalidYenEncoding:encoding]) {
...

{
    ...
    if ([self isDocumentEdited]) {
        messageText = @"The file has been modified by another process. There are also unsaved changes in TryEditor.";
        informativeText = @"Do you want to keep TryEditor's edition or update to the modified edition?";
        defaultButton = @"Keep TryEditor's Edition";
    } else {
